...
- **Notification doc clarifications + `altType`/`altId` revert** ([`e957e1a`](https://github.com/si-commits/Go-High-Level-MCP-2026-Complete/commit/e957e1a), tool descriptions [`d2044eb`](https://github.com/si-commits/Go-High-Level-MCP-2026-Complete/commit/d2044eb)): documentation-only polish to the notification surface. No behavioural change.
  ...
  - **Smoke-test finding (notifications are append-only via the API):** the polish-pass probes (`wrapper-polish-smoke.md`) confirmed afterTime round-trips and formId persists through updates, and surfaced that deleting a calendar only soft-cascades its inApp notifications: the records flip to `deleted: true` but survive the parent deletion, still resolvable by id. Combined with the prior soft-delete and hard-delete probes, this means notification records are never reclaimed by the API, so clean reprovisioning needs the GHL UI. See `wrapper-polish-smoke.md` for detail.
